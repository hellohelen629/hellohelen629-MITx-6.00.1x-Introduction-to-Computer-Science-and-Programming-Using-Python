...
def getWordScore(word, n):
    ...
    letter_score = 0
    for i in word:
        letter_score += SCRABBLE_LETTER_VALUES[i]
    word_score = letter_score*len(word)
...
def updateHand(hand, word):
    ...
    for e in word:
        hand_updated[e] = hand_updated.get(e) - 1
...
def isValidWord(word, hand, wordList):
    ...
    #    return False
        ...
        boo = []
        for e in word_dict.keys():
            if word_dict.get(e) > hand.get(e,0):
                return False
            elif e not in hand.keys():
                 boo.append(False)
        if False in boo:
            return False
        else:
...
def playGame(wordList):
    ...
    n = HAND_SIZE
    while True:
        enter = input('Enter n to deal a new hand, r to replay the last hand, or e to end game: ')
        if enter == 'e':
            break
        elif enter == 'n':
             hand = dealHand(n)
